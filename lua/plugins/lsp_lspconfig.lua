...
local function load_server_config()
  ...
  if has_cmp_nvim_lsp then serverconfig.capabilities = cmp_nvim_lsp.default_capabilities(serverconfig.capabilities) end
...
M.lazy = {
  ...
  config = function() load_server_config() end,
  keys = {
    { "gd", mode = { "n" }, vim.lsp.buf.definition, desc = "Lsp Goto Definition" },
    {
      "gX",
      mode = { "n" },
      ":rightbelow split | lua vim.lsp.buf.definition()<CR>",
      desc = "Lsp Horizontal Split Goto Definition",
    },
    {
      "gV",
      mode = { "n" },
      ":rightbelow vsplit | lua vim.lsp.buf.definition()<CR>",
      desc = "Lsp Vertical Split Goto Definition",
    },
