...
--  We currently ship most of them with nvim.
{
  ...
  keys = {
    ...
    { "<F11>", mode = { "n", "x" }, function() require('dap').clear_breakpoints() end, desc = "Debugger Clear Breakpoint" },
    { "<F24>c", mode = { "n", "x" }, function()
      vim.ui.input({ prompt = "Condition: " }, function(condition)
        if condition then require("dap").set_breakpoint(condition) end
      end)
    end, desc = "Debugger Breakpoint Condition" },
    { "<F24>l", mode = { "n", "x" }, function()
      vim.ui.input({ prompt = "Message: " }, function(condition)
        if condition then require("dap").set_breakpoint(nil, nil, condition) end
      end)
    end, desc = "Debugger Log Point Message" },
    -- widgets
    { "<leader>ds", mode = { "n", "x" }, function()
      local widgets = require('dap.ui.widgets')
      require('dap.ui.widgets').sidebar(widgets.scopes).open()
    end, desc = "Debugger Toggle Scope View (Only)" },
  }
}
